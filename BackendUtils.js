...
class UserModel {
  ...
  static async create(deviceId, platformData = {}) {
    ...
    const user = {
      ...
      version: platformData.Version || '0.56',
      createdAt: now,
      lastLogin: now,
      newsVersion: 0,
      skillRating: 0,
  ...
  static async findByBanIdentifier(identifier) {
    if (!identifier) return null;

    const direct = await database.getUserByQuery({
        $or: [
            { deviceId: identifier },
            { stumbleId: identifier }
        ]
    });

    if (direct) return direct;

    const users = await database.collections.Users.find({
    ...
    for (const user of users) {
        const hash = crypto
            .createHash('sha1')
            .update(String(user.deviceId))
            .digest('hex');

        if (
            hash === String(identifier) ||
            hash.toUpperCase() === String(identifier)
        ) {
            return user;
        }
    }
...
class UserController {
  static async login(req, res) {
    try {
      ...
      if (!DeviceId) {
        Console.error("Login", "DeviceId missing");
        return res.status(400).json({ message: 'DeviceId is required' });
      }

      let user = null;
      if (StumbleId) {
        Console.log("Login", "Finding user by StumbleId:", StumbleId);
        user = await UserModel.findByStumbleId(StumbleId);
      }

      if (!user) {
        Console.log("Login", "Finding user by DeviceId:", DeviceId);
        user = await UserModel.findByDeviceId(DeviceId);
      }

      if (!user) {
        Console.log("Login", "Creating new user with DeviceId:", DeviceId);
        user = await UserModel.create(DeviceId, { steamTicket: SteamTicket, scopelyId: ScopelyId, Version });
        Console.log("Login", "User created successfully:", user.stumbleId);
      }

      if (user.isBanned) {
        Console.log("Login", "User is banned:", user.stumbleId);
        return res.status(403).send("BANNED");
      }

      Console.log("Login", "Generating Photon JWT for user:", user.stumbleId);
      const photonJwt = await generatePhotonJwt(user);

      Console.log("Login", "Login successful for user:", user.stumbleId);
      
      // Feature flags for mobile client
      const featureFlags = [
        "Consensus",
        "CustomParty",
        "EndOfMatchRewardedVideo",
        ...
        "GraphicsQualitySettings",
        "SpecialEmoteFilter",
        "AssetDownloadSystem" // للـ download assets
      ];
      ...
      return res.status(200).json({
        ...
        DownloadAssetsVersion: SharedData.DownloadAssetsVersion || 0.5
      });
    } catch (err) {
  ...
  static async getConfig(req, res) {
    try {
      const config = {
        _SharedVersion: 2,
        Versions: {
          AndroidLastVersionAvailable: 0.56
        },
...
async function sendShared(req, res) {
  try {
    Console.log("Shared", `Request: ${req.url}`);
    
    // Send shared.json as response
...
async function getAppId(req, res) {
  const appId = process.env.PHOTON_APP_ID || "01fcebe1-9639-497c-9892-5125ba345fc4";
  const encryptionKey = crypto
    .createHash("sha256")
    .update(process.env.LeagueSalt || "cOrCzAln3JfVZ5tgeM2m0QvwvbM3sp4H")
    .digest();
  ...
  Console.log("Photon", `Sending App ID: ${appId}`);
